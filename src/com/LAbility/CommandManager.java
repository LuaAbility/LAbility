			...
			if (args.length > 0) {
				...
				if (args[0].equalsIgnoreCase("check")) {
					...
					for (LAPlayer lap : main.gameManager.players) {
						if (lap.player.equals(player)) {
							if ((args.length > 1)) {
								lap.CheckAbility(player, Integer.parseInt(args[1]));
								return true;
							} else {
								lap.CheckAbility(player, -1);
								return true;
							}
						}
					}
					player.sendMessage("\2474[\247cLAbility\2474] \247c게임에 참여 중이 아닙니다.");
				}

				if (args[0].equalsIgnoreCase("ob")) {
					if ((args.length > 1)) {
						for (Player p : main.getServer().getOnlinePlayers()) {
							if (p.getName().equals(args[1]) && main.gameManager.players.contains(p)){
				...
				if (args[0].equalsIgnoreCase("ablist")) {
					...
					for (int i = 0; i < 8; i++) {
						...
						sender.sendMessage("\2476" + targetIndex + ". \247e" + main.abilities.get(targetIndex).abilityName);
					}
				}
				...
				if (args[0].equalsIgnoreCase("ability")) {
					if ((args.length > 1)) {
						...
						if (-1 < id && id < main.abilities.size()) main.abilities.get(id).ExplaneAbility(player);
						else sender.sendMessage("\2474[\247cLAbility\2474] \247c존재하지 않는 ID 입니다.");
					}
				}
			}
		}
